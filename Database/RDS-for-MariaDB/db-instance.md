...
백업을 이용하여 새로운 DB 인스턴스를 생성할 수 있습니다. 백업을 수행한 DB 인스턴스와 바이너리 로그(binary log)가 존재한다면 특정 시점 혹은 원하는 바이너리 로그(binary log) position으로 복원이 가능합니다. RDS for MariaDB이 아닌 외부 MariaDB의 백업으로도 복원이 가능합니다. 복원 시 항상 새로운 DB 인스턴스가 생성되며, 기존 DB 인스턴스의 데이터베이스를 지우고 복원할 수 없습니다. 복원에 대한 자세한 설명은 [백업 및 복원](backup-and-restore/) 항목을 참고합니다.
...
장애 조처된 마스터 복구에 실패하여 데이터 보정이 필요할 경우 장애 조처된 마스터를 분리하여 고가용성 기능을 비활성화할 수 있습니다. 분리된 마스터와 승격된 마스터 간의 복제 관계가 끊어지며 각각 일반 DB 인스턴스로 동작합니다. 분리된 이후에는 다시 원래 구성으로 복구가 불가능합니다.
