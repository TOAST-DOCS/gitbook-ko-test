## Database > RDS for MariaDB > 개발자 가이드

## Migration

* RDS는 mysqldump를 이용하여 NHN Cloud RDS 의 외부로 데이터로 내보내거나 외부로부터 가져올 수 있습니다.
* mysqldump 유틸리티는 MariaDB를 설치했을 때 기본으로 제공됩니다.
...
* NHN Cloud RDS 버전은 외부 MariaDB 버전과 같거나 그보다 최신 버전이어야 합니다.
...
* RDS for MariaDB의 백업 파일을 오브젝트 스토리지로 내보내거나, 오브젝트 스토리지의 백업 파일을 이용하여 DB 인스턴스를 복원할 수 있습니다.

> [주의] 오브젝트 스토리지의 백업 파일과 복원하려는 MariaDB의 버전은 동일해야 합니다.

### 오브젝트 스토리지에 백업 내보내기

* NHN Cloud의 오브젝트 스토리지에 RDS for MariaDB의 백업을 내보낼 수 있습니다.
...
* 오브젝트 스토리지의 백업 파일을 이용하여 직접 MariaDB을 복원할 수 있습니다.
* 복원할 MariaDB 및 Mariabackup이 설치되어 있다고 가정합니다.
* 오브젝트 스토리지의 백업을 복원하고자 하는 서버에 다운로드합니다.
* MariaDB 서비스를 정지합니다.
* MariaDB 데이터 저장 경로의 모든 파일을 삭제합니다.
...
### 오브젝트 스토리지의 외부 MariaDB 백업 파일을 이용하여 DB 인스턴스 생성

* 일반 MariaDB 백업 파일을 이용하여 RDS for MariaDB의 DB 인스턴스로 복원할 수 있습니다.
...
* RDS for MariaDB는 사용자의 편의를 제공하기 위하여 사용자 계정에서 제한되는 몇몇 기능들을 수행하는 프로시저들을 자체적으로 제공하고 있습니다.
...
* 파라미터 설명
    ...
    * master_instance_port : 복제 대상(Master) 서버의 MariaDB Port
    * user_id_for_replication : 복제 대상(Master) 서버의 MariaDB 에 접속 할 복제용 계정
