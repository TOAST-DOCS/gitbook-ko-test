...
* 오브젝트 스토리지 백업으로 복원 시 백업 타입 오류가 발생하면 이벤트에 기록되도록 개선

#### 버그 수정

* 오브젝트 스토리지 백업으로 복원 시 버전 오류 발생에 대한 이벤트가 기록되지 않는 문제 수정

### 2023. 02. 14.
#### 기능 개선

* MS-SQL 모니터링 활성화 여부를 선택할 수 있는 기능 추가

### 2023. 01. 10.
#### 기능 개선

* 오브젝트 스토리지로 전체 백업 기능 제거
* 오브젝트 스토리지로 차등 백업 시 전체 백업을 같이 내보낼 수 있도록 기능 개선
* 오브젝트 스토리지로 차등 백업 시 현재 데이터베이스 기준으로 선택할 수 있도록 기능 개선
* 오브젝트 스토리지에 있는 백업으로 복원 시 차등 백업을 같이 복원할 수 있도록 기능 개선
* 오브젝트 스토리지로 백업 내보내기 시 백업된 데이터베이스를 선택할 수 있도록 기능 개선

#### 버그 수정

* 오브젝트 스토리지에 있는 백업으로 복원 시 차등 백업은 복원 불가능한 현상 수정 

### 2022. 12. 13.
#### 기능 개선

* 여러 개의 보안 그룹을 선택하여 삭제할 수 있도록 수정

#### 버그 수정

* 에러 로그 조회 시 로그의 길이가 너무 길면 에러가 발생하는 현상 수정 

### 2022. 11. 15.
#### 기능 개선
* 로그 백업에 실패했을 때 사용자가 자동 백업을 진행할 수 있는 기능 추가
* 자동 백업 및 로그 백업을 활성화하거나 비활성화할 수 있는 기능 추가
* Temp DB의 위치를 사용자가 선택할 수 있는 기능 추가


### 2022. 10. 11.
#### 버그 수정
* 에러 로그를 최대 3개월 단위로 조회할 수 있도록 제한

### 2022. 09. 14.

#### 기능 개선

* 재시작이 필요한 파라미터 변경 시 DB 인스턴스 재시작이 예약되지 않도록 수정
* 에러 로그 조회 시 무한 스크롤로 확인할 수 있도록 개선

#### 버그 수정

* 싱글 DB 인스턴스에 데이터베이스 생성 직후 고가용성으로 수정 시 데이터베이스 복제가 2회 발생하는 현상 수정

### 2022. 08. 09.

#### 기능 개선

* 잘못된 파라미터 변경으로 인해 DB 접속 불가 상태가 되더라도 파라미터 변경할 수 있도록 수정

#### 버그 수정

* 간헐적으로 장애 조치를 실패하는 문제 수정

### 2022. 07. 12.

#### 기능 개선

* MS-SQL의 시스템 메모리 부족으로 백업 실패 시 원인을 이벤트에 남기도록 개선
* 콘솔에서 로그 보기 화면이 느린 현상 개선

#### 버그 수정

* 선택된 파라미터 그룹 복제 시 선택 해제되도록 수정
* 간헐적으로 로그 백업이 실패하는 문제 수정
* 로그 백업이 시작 전에 실패할 경우 전체 백업하지 않도록 수정

### 2022. 06. 14.

#### 기능 개선

* 보안 그룹 규칙에 송신 방향 추가할 수 있도록 개선
* 보안 그룹 규칙에 포트 추가할 수 있도록 개선
* SQL Server 2019 Standard (15.0.4223.1) 버전 추가
* DB 인스턴스에 적용된 파라미터 그룹 버전이 낮을 시 적용할 수 있도록 개선

#### 버그 수정

* DB 보안 그룹의 이름만 변경 시 실패하는 문제 수정
* 상품 비활성화 시 간헐적으로 실패하는 문제 수정
...
* numeric 파라미터에 `,`를 넣을 수 있는 버그 수정

### 2022. 01. 11.

#### 버그 수정

* 간헐적으로 DB 인스턴스 생성에 실패하는 문제 수정

### 2021. 12. 14.

#### 기능 개선

* 인스턴스 생성 시, 서브넷을 선택할 때 CIDR뿐 아니라 이름도 노출하도록 개선
* master 데이터베이스의 데이터 경로를 데이터 볼륨으로 변경

#### 버그 수정

* 이벤트 구독 등록 혹은 수정 시, 구독 불가능한 이벤트 코드를 선택할 수 있던 버그 수정
* DB 인스턴스 삭제 시, 간헐적으로 불필요한 스토리지가 남아 있던 버그 수정
* DB 인스턴스 재시작 후 간헐적으로 접속 불가 상태로 변경되는 현상 수정
* DB 인스턴스 수정 시, 간헐적으로 작업이 완료되지 않았음에도 잠깐 사용 가능 상태로 변경되는 현상 수정
* 서버 대시보드에서 디스크 전송률이 잘못 표기되는 문제 수정

### 2021. 11. 09.

#### 기능 추가

* 예정된 하이퍼바이저 마이그레이션에 대한 알림을 이메일과 SMS로 발송하는 기능 추가

#### 버그 수정

* CloudTrail의 응답 본문이 긴 경우 잘리는 현상 수정
