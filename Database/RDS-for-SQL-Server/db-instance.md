...
DB 인스턴스의 모니터링 상태는 아래와 같은 값들로 구성되며, 사용자의 행위와 현재 상태에 따라 변경됩니다.

| 상태    | 설명                            |
| ------- |-------------------------------|
| MS-SQL 모니터링 항상 활성화 | MS-SQL 모니터링이 활성화된 상태          |
| MS-SQL 모니터링 특정 시간 비활성화 | MS-SQL 모니터링이 특정 시간에만 비활성화되는 상태 |
| MS-SQL 모니터링 비활성화 | MS-SQL 모니터링이 비활성화된 상태         |
...
DB 인스턴스 수정, 파라미터 그룹 변경시 등록되는 DB 인스턴스 재시작 등 예약 작업이 예약되는 시간입니다.

> [주의]
> 설정된 시간 안에 작업이 완료되지 않을 수 있습니다.
> 설정된 시간에 다른 작업이 동작 중이면 다음날 예약 시간에 재시도합니다.
...
고가용성 자동 복구 기능은 자동 장애 조치 완료 1시간 후에 동작하는 고가용성 재구성 작업을 예약합니다. 생성된 예약 작업은 **예약 작업** 탭에서 원하는 시간으로 수정 혹은 삭제할 수 있습니다.
