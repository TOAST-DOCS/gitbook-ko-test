...
### 메일 보내기 
#### 보내는 주소와 이름
보내는 주소와 이름을 변경하여 메일을 발송할 수 있습니다. [ 설정 > 메일 > 쓰기 ] (아래 화면)에서 보내는 주소/이름을 각각 최대 10개까지 추가할 수 있고, 추가된 주소와 이름은 쓰기 창에서 자유롭게 선택하여 메일 발송할 수 있습니다. 
![](http://static.toastoven.net/prod_dooray_mail/Mail_01_ko.png)

#### 개별 발송
메일을 여러 명에게 동시에 보내도 받은 메일에서 개인 1명만 수신인으로 보여지도록 개별 발송할 수 있습니다. 메일 쓰기 창에서 받는 사람 입력란의 우측에 [개별]을 체크하고 메일을 발송합니다.

#### 수신, 참조, 숨은 참조
메일 쓰기 창에서 받는 사람 외에도, 참조와 숨은 참조로 메일 주소를 넣을 수 있습니다. 
숨은 참조는 메일을 받는 사람과 참조인에게는 메일 주소가 노출되지 않습니다. 숨은 참조로 메일을 받은 사람도 수신인에 본인의 메일 주소가 노출되지 않으므로 업무 메일을 보낼 때 참조와 숨은 참조를 구분할 필요가 있습니다. 
숨은 참조로 보낸 메일도 보낸 메일 목록 또는 본문에서 수신 여부를 확인할 수 있습니다. 

#### 보안 메일
외부 계정으로 메일 발송을 금지하고 특정 메일에 대해 일정 기간 후 자동으로 삭제 또는 전달을 금지할 수 있습니다. 
쓰기 창 안의 [보안]버튼을 클릭해 일반, 대외비, 기밀 중 선택 / 설정합니다. 
대외비로 설정 시, 메일 전달이 금지되고 설정한 기간(기본 30일)이 지난 후 메일이 자동 삭제 됩니다. 기밀로 설정 시, 메일 전달이 금지되고 설정한 기간 (기본 30일)이 지나면 자동 삭제 되도록 설정됩니다. 
보안 메일 전송 시, 수신 주소에 외부 계정이 포함된 경우 외부 계정을 수신인에서 제외해야 메일이 발송됩니다. 참고로, 보안 메일은 업무로 등록할 수 없습니다.
![](http://static.toastoven.net/prod_dooray_mail/Mail_02_ko.png)

#### 첨부 파일
최대 20MB까지 일반 첨부 가능하며, 대용량 첨부(링크 형태)로는 최대 5GB까지 가능합니다. 대용량 첨부 메일은 30일간 보관되며 이후 자동 삭제됩니다.
...
#### 메일 보관
받은 메일을 보관하고자 하는 경우 메일 상단의 [보관]을 선택합니다. 자동 분류 기능을 활용해 메일 제목, 보낸 사람, 받는 사람, 참조, 첨부파일 유무 등의 기준으로 보관 메일함에 자동으로 보낼 수 있습니다.
...
![](http://static.toastoven.net/prod_dooray_mail/Mail_03_ko.png)

### 수신 확인 및 발송 취소 
발송한 메일을 수신인이 읽었는지 확인을 하고 싶거나, 발송 취소를 하고 싶다면 보낸 메일함의 목록이나 보낸 메일의 본문에서 확인하세요. 

![](http://static.toastoven.net/prod_dooray_mail/Mail_04_ko.png)

수신인 옆에 [2/3명 읽음▼]으로 간단하게 수신 여부를 확인할 수 있고, 해당 영역을 클릭하면 개별의 수신 상태 및 읽은 시간 등과 같이 자세한 정보를 추가로 확인할 수 있습니다. 단, 텍스트 모드와 SMTP로 메일을 발송한 경우에는 수신 확인이 불가능 할 수 있습니다.
같은 테넌트(회사/조직) 사용자에 보낸 메일에 대해서는 필요한 경우 수신 확인 정보 옆의 [발송 취소]를 클릭해 보낸 메일을 회수할 수 있습니다. (외부 메일 계정으로 발송된 메일은 회수가 불가능합니다.) 수신인이 읽기 전이거나 발송 후 24시간 이내에만 가능하고, SMTP로 발송한 메일은 회수가 불가능합니다. 

### 받은 메일 업무로 등록 
메일 내용을 업무로 바로 업무로 등록할 수 있습니다. 메일 본문 상단의 [업무로 등록]을 선택합니다. (단, 대외비/기밀로 설정된 보안 메일은 업무 등록 불가)

![](http://static.toastoven.net/prod_dooray_mail/Mail_05_ko.png)

외부에서 온 메일을 업무로 반복해서 등록해야 하는 경우, [자동 분류] 기능을 활용해 보세요. 제목, 보낸 사람, 받는 사람, 참조 등의 기준으로 특정 프로젝트와 담당자를 지정하여 자동으로 업무를 등록할 수 있습니다.

![](http://static.toastoven.net/prod_dooray_mail/Mail_06_ko.png)

### 주소록에 추가 
받은 메일에 있는 메일 주소를 주소록에 간편하게 추가할 수 있습니다. 
메일 주소를 클릭할 경우 선택한 사람의 프로필이 노출됩니다. 주소록에 추가되지 않은 사람의 경우 프로필 우측 하단에 [주소록 추가] 메뉴가 노출되며, 이미 주소록에 추가된 사람의 경우 [주소록 편집] 메뉴가 노출됩니다. 주소록에 한번 추가하면, 이후 메일 쓰기를 할 때 메일 주소를 일일이 입력하지 않아도 자동 완성되어 편리하게 메일을 작성할 수 있습니다.

![](http://static.toastoven.net/prod_dooray_mail/Mail_07_ko.png)
...
### 조직도를 활용한 메일 발송
조직도 ‘사용함’ 상태인 경우 [조직도] 메뉴를 클릭해 조직 내 멤버/부서를 선택해 메일을 발송할 수 있습니다.

![](http://static.toastoven.net/prod_dooray_mail/Mail_09_ko.png)

조직도 설정은 조직 관리자가 [조직 관리 > 부서/멤버 관리 > 조직도 관리]에서 할 수 있습니다. 

![](http://static.toastoven.net/prod_dooray_mail/Mail_10_ko.png)

### 메일 검색 
서비스 우측 상단의 검색 영역에서 키워드를 입력하여 메일을 검색할 수 있습니다. 전체 메일함 뿐 아니라 메일함 별, 폴더 별 검색이 가능합니다.
...
![](http://static.toastoven.net/prod_dooray_mail/Mail_11_ko.png)
...
#### 읽기> 목록 개수
메일 읽기 시 목록 한 페이지에 노출되는 메일 개수를 설정 할 수 있습니다. 메일 개수는 10개, 15개, 30개, 50개, 100개, 200개 중 설정할 수 있습니다.
...
(참고) POP3(post office protocol)/ IMAP(Internet messaging access protocol) 소개
...
#### 서명 설정 
[설정> 메일> 서명]을 선택하여 사용함으로 선택을 하고 새로 작성하는 메일에 자동 포함할 지, 전달, 답장(전체 답장)에 자동으로 포함할지를 선택합니다. 서명은 최대 3개까지 추가할 수 있으며 기본 서명을 지정할 수 있습니다.
...
1) 서명으로 활용하고 싶은 이미지를 (반드시 공개된) 블로그, 웹페이지 등에 업로드 
2) 서비스에 올라간 이미지를 우클릭 해서 이미지 주소 복사 
3) 서명 편집창에서 복사한 이미지 주소 입력 / 서명 완성 

#### 부재 중 설정 및 부재 중 자동 답장 
부재 중 설정과 부재 중 자동 답장을 설정할 수 있습니다. 부재 기간을 설정하고, 답장 내용을 작성해 보세요. 부재 기간 동안 내가 작성해둔 내용으로 답장이 발송됩니다.  
...
스팸을 해제하고자 할 경우에는, 메일 본문 상단의 스팸 해제 버튼을 눌러 해제할 수 있으며, 해당 메일은 받은 메일함으로 이동되고 해당 메일 주소를 수신 허용 목록에 추가할 수 있습니다.

### 메일 가져오기
다른 외부 메일을 .eml(또는 eml 압축 파일)형태로 Dooray! 메일에 가져올 수 있습니다. (가져오기는 한번에 4G 이하만 가능)
...
### 메일 번역 기능 
메일을 받는 사람의 언어 설정에 따라 메일 본문을 바로 번역해서 볼 수 있도록 번역 기능을 제공합니다. Dooray! 메일 수신자의 언어 설정은 [설정> 일반> 언어]에서 확인할 수 있으며, 메일을 받은 경우 본문 위의 메뉴 영역에서 […] 메뉴를 누른 후, [번역 켜기] 메뉴를 누르면 메일 수신인의 언어 설정을 인식해 번역될 언어가 셋팅 됩니다. 번역할 언어를 확인 후 [번역하기] 버튼을 한 번만 누르면 됩니다. 기본 언어 설정을 영어로 한 사용자의 좌측 메일에서 번역 켜기 메뉴를 누르면 우측과 같이 제목부터 본문까지 모두 영어로 번역이 됩니다.
...
2. 이미 조직을 만드셨다면 테넌트 소유자 또는 테넌트 관리자가 [조직관리 > 공통 설정 > 도메인]에서  [자체 메일 도메인 추가] 버튼을 눌러 자체 도메인을 추가할 수 있습니다.(아래 화면 참고)

![](http://static.toastoven.net/prod_dooray_mail/Mail_17_ko.png)
<center>[그림]자체 도메인 추가</center> 자체 도메인 추가

자체 도메인 추가 시 MX레코드 및 SPF레코드 설정은 아래 안내를 참고해서 진행해주세요. 
참고로, 조직관리에서 자체 도메인 추가는 1개만 가능합니다.  2개 이상의 도메인을 추가하고 싶다면,  dooray@nhn.com으로 문의주세요.
...
(※ 아래 등록 양식 입력시 include:와 _ 사이 공백 없도록 주의! all 뒤에 공백 없도록 주의!)
